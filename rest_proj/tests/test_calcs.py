...
# client creates a post with its own session whilie i have my session and together those 2 dont  go together. 
# any changes i make to my own post are fucked.
...
def test_post(authorized_clients, title, con, session):
    ...
    post_details_from_sql = ReturnPost( **session.execute(text("select * from posts where id = :id"), 
                                                    {"id" : post_details_from_resposne.id}).mappings().fetchone()  )
...
def test_update_post(authorized_clients, create_post_for_update, session):
    ...
    res = authorized_client.put( f"/posts/{id}", json={"title": 'soon to be updated!', "con" : 'soon to be updated contents'  } )
    # print()
    assert res.status_code == status.HTTP_200_OK
    assert res.json()['message'] == 'modified post!'
    assert session.execute(text("select title, con from posts where id = :id"), {"id" : id}).fetchone() == ('soon to be updated!', 'soon to be updated contents'  )
